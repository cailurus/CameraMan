...
def run(
    model: str, max_results: int, score_threshold: float, overlapping_factor: float
) -> None:
    ...
    options = audio.AudioClassifierOptions(
        ...
        category_allowlist=["Speech", "Singing", "Typing", "Clapping"],
        display_names_locale="cn",
    )
    ...
    while True:
        ...
        if classification_result_list:
            ...
            if classification.categories:
                result = classification.categories[0].category_name
            else:
                result = "Nothing"

            print(result)
            f = open("audio_res.txt", "w")
            f.write(result)
